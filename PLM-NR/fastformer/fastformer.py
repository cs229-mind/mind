...
class FastformerEncoder(nn.Module):
    def __init__(self, args, hidden_size, intermediate_size, config=None, pooler_count=1):
        super(FastformerEncoder, self).__init__()
        self.args = args
        if config is None:
            config = BertConfig.from_json_file(os.path.join(os.path.expanduser(self.args.model_dir.rsplit('/', 1)[0]), 'fastformer/fastformer.json'))
