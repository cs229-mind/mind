...
def ndcg(y_true, y_pred, ats=None, gain_function=lambda x: torch.pow(2, x) - 1, padding_indicator=PADDED_Y_VALUE,
         filler_value=1.0):
    """
    ...
    """
    y_true = reshape(y_true, y_pred)

    idcg = dcg(y_true, y_true, ats, gain_function, padding_indicator)
    ndcg_ = dcg(y_true, y_pred, ats, gain_function, padding_indicator) / idcg
    idcg_mask = idcg == 0
...
def __apply_mask_and_get_true_sorted_by_preds(y_true, y_pred, padding_indicator=PADDED_Y_VALUE):
...
def dcg(y_true, y_pred, ats=None, gain_function=lambda x: torch.pow(2, x) - 1, padding_indicator=PADDED_Y_VALUE):
    """
    ...
    true_sorted_by_preds = __apply_mask_and_get_true_sorted_by_preds(y_true, y_pred, padding_indicator)
...
def mrr(y_true, y_pred, ats=None, padding_indicator=PADDED_Y_VALUE):
    """
